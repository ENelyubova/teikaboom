...
class NewsBackendController extends yupe\components\controllers\BackController
{
    ...
    public function actionDeleteImage()
    {
        if (Yii::app()->getRequest()->getIsPostRequest() && Yii::app()->getRequest()->getIsAjaxRequest()) {
            ...
            $model = NewsImage::model()->findByPk($id);
    ...
    public function updatePagePhotos(News $News)
    {
        if (Yii::app()->getRequest()->getPost('NewsImage')) {
            foreach (Yii::app()->getRequest()->getPost('NewsImage') as $key => $val) {
                $NewsImage = NewsImage::model()->findByPk($key);
                if (null === $NewsImage) {
                }
                $NewsImage->setAttributes($_POST['NewsImage'][$key]);
                if (false === $NewsImage->save()) {
                    Yii::app()->getUser()->setFlash(\yupe\widgets\YFlashMessages::ERROR_MESSAGE,
                        Yii::t('ParkModule.park', 'Error uploading some images...'));
                }
            }
        }
        foreach (CUploadedFile::getInstancesByName('NewsImage') as $key => $image) {
            $NewsImage = new NewsImage();
            $NewsImage->news_id = $News->id;
            $NewsImage->attributes = $_POST['NewsImage'][$key];
            $NewsImage->addFileInstanceName('NewsImage[' . $key . '][image]');
            $NewsImage->save();
        }
    }
}
