...
class Product extends yupe\models\YModel implements ICommentable
{
    ...
    public function relations()
    {
        return [
            ...
            'productsRelation' => [self::MANY_MANY, 'StoreCategory', 'yupe_store_product_category(category_id, product_id)'],
            'badgesProduct' => [self::MANY_MANY, 'ProductBadge', 'yupe_store_product_badge(badge_id, product_id)', 'index' => 'id'],
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'badges' => Yii::t('StoreModule.store', 'Badges:'),
        ];
    }
    ...
    {
        $badges = ProductBadge::model()->findAllByAttributes(['product_id' => $this->id]);
        foreach ($badges as $key => $badge) {
            $this->badges[] = $badge->badge_id;
        }
    }
    ...
    {
        return CHtml::listData(Badge::model()->published()->findAll(), 'id', 'name');
    }

    public function getProductBadgeList()
    {
        $badges = ProductBadge::model()->findAllByAttributes(['product_id' => $this->id]);
        foreach ($badges as $key => $badge) {
            $this->badges[] = $badge->badge_id;
        ...
        $product_badges = [];
        foreach (Badge::model()->findAllByAttributes(['id' => $this->badges]) as $key => $item) {
            $product_badges[] = [
                'color' => $item->color,
    ...
    public function saveBadges(array $badgesId)
    {
        ProductBadge::model()->deleteAllByAttributes(['product_id' => $this->id]);
        foreach ($badgesId as $key => $badge) {
            $model = new ProductBadge;
            $model->badge_id = $badge;
    ...
    public function getShortDescription($length, $postfix='...', $encoding='UTF-8')
    {
      if (mb_strlen($this->short_description, $encoding) <= $length) {
        return $this->short_description;
      }

      $tmp = mb_substr($this->short_description, 0, $length, $encoding);
      return mb_substr($tmp, 0, mb_strripos($tmp, ' ', 0, $encoding), $encoding) . $postfix;
    }

    public function getVisibleButton($length, $encoding='UTF-8')
    {
      if (mb_strlen($this->short_description, $encoding) <= $length) {
        return false;
      }
    ...
    {
        if (mb_strlen($this->name, $encoding) <= $length) {
            return $this->name;
        }

        $tmp = mb_substr($this->name, 0, $length, $encoding);
        return mb_substr($tmp, 0, mb_strripos($tmp, ' ', 0, $encoding), $encoding) . $postfix;
    }
}
