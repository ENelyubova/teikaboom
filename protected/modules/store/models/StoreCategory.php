...
class StoreCategory extends \yupe\models\YModel
{
    ...
    public function rules()
    {
        return [
            [
                'name, name_short, title, description, short_description, slug, meta_title, meta_keywords, meta_description, is_home',
                'filter',
                'filter' => 'trim',
            ],
            ['name, name_short, slug', 'filter', 'filter' => [$obj = new YPurifier(), 'purify']],
            ['name, name_short, slug', 'required'],
            ['parent_id, status, sort, is_home', 'numerical', 'integerOnly' => true],
            ['parent_id, status', 'length', 'max' => 11],
            ['parent_id', 'default', 'setOnEmpty' => true, 'value' => null],
            ['status', 'numerical', 'integerOnly' => true],
            ['status', 'length', 'max' => 11],
            ['name, name_short, title, image, image_alt, image_title, meta_title, meta_keywords, meta_description, meta_canonical', 'length', 'max' => 250],
            ['slug', 'length', 'max' => 150],
            ...
            ['meta_canonical', 'url'],
            ['is_home', 'boolean'],
            ['name_short, name_desc, is_home', 'safe'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'is_home' => Yii::t('StoreModule.store', 'Выводить на главной'),
        ];
    }
    ...
    public function renMenu($items, $level = 0, $parent = null)
    {
        $menu = '';
        ...
        foreach ($items as $item) {
            ...
            if (!empty($item['items'])) {
                $parent = $item;
                $menu .= $this->renMenu($item['items'], $level + 1, $parent);
            }
